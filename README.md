...
这是一个基于人工智能算法的情感计算与音乐推荐工具。系统能够通过多路文本情感模型识别用户心理状态，并在 Valence-Arousal (V-A) 空间内实时匹配最合适的音乐表。

## 📺 项目演示

![Demo Video](demo.mp4)

> **注**：如果您的 Markdown 阅读器不支持视频直传播放，请直接打开 [demo.mp4](demo.mp4) 文件查看。
...
- **情感回归任务**：模型后端连接稠密层 (Dense Layers)，将高维语义特征压缩至二维向量，输出预测的愉悦度 ($V$) 与唤醒度 ($A$) 坐标（取值范围均为 $[-1, 1]$）。
...
- **方案 A：高精度自动化标注（当前稳定驱动）**
  - **实现逻辑**：利用 Playwright 自动化处理流，将本地 MP3 上传至部署有音频数字信号处理 (DSP) 算法的 [Essentia.js 环境](http://mtg.github.io/essentia.js/examples/#/demos/)。
